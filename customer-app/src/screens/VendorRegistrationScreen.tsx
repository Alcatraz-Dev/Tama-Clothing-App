...
export const VENDOR_TIERS: VendorTierInfo[] = [
  ...
  },
  {
    ...
    benefits: [
      "featureLowCommission",
      "featureFeaturedListing",
      "featurePriorityDelivery",
    ],
  },
  {
    id: "enterprise",
...
export default function VendorRegistrationScreen({
  ...
  const [paymentMethod, setPaymentMethod] = useState<"bank" | "post" | null>(
    null,
  );
  const [showPaymentDetails, setShowPaymentDetails] = useState(false);
  const [paymentProofForApproval, setPaymentProofForApproval] = useState<
    string | null
  >(null);
  ...
  const [activePickerType, setActivePickerType] = useState<
    "license" | "idFront" | "idBack" | "front" | "payment" | null
  >(null);
  ...
  const uploadImage = async (uri: string): Promise<string> => {
    const result = await uploadToSanity(uri);
    return result || "";
  };

  // Media choice modal handlers - using MediaPicker component
  const handleMediaSelection = async (assets: MediaAsset[]) => {
    if (assets.length === 0 || !activePickerType) return;
    const uri = assets[0].uri;
    setIsMediaModalVisible(false);

    if (activePickerType === "license") setBusinessLicense(uri);
    if (activePickerType === "idFront") setIdCardFront(uri);
    if (activePickerType === "idBack") setIdCardBack(uri);
    if (activePickerType === "front") setStoreFront(uri);
    if (activePickerType === "payment") setPaymentProof(uri);
  };

  const handleCameraResult = async (uri: string, type: "image" | "video") => {
    setViewMode("form");
    if (uri && activePickerType) {
      if (activePickerType === "license") setBusinessLicense(uri);
      if (activePickerType === "idFront") setIdCardFront(uri);
      if (activePickerType === "idBack") setIdCardBack(uri);
      if (activePickerType === "front") setStoreFront(uri);
  ...
  const pickImage = async (
    type: "license" | "idFront" | "idBack" | "front" | "payment",
  ) => {
    ...
    switch (currentStep) {
      ...
      case 2:
        ...
        const hasCustomCategory =
          businessCategory === "other" ? !!customCategory : true;
        return (
          !!accountType &&
          !!businessName &&
          !!businessEmail &&
  ...
  const handleNext = () => {
    if (!validateStep(step)) {
      if (step === 3) {
        ...
        Alert.alert(
          t("error") || "Error",
          t("vendorFillAllFields") || "Please fill all required fields",
        );
      }
      return;
    }
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (businessLicense)
        licenseUrl = businessLicense.startsWith("http")
          ? businessLicense
          : await uploadImage(businessLicense);
      if (storeFront)
        frontUrl = storeFront.startsWith("http")
          ? storeFront
          : await uploadImage(storeFront);

      let paymentProofUrl = null;
      if (paymentProof)
        paymentProofUrl = paymentProof.startsWith("http")
          ? paymentProof
          : await uploadImage(paymentProof);
      ...
      const vendorData = {
        ...
        paymentMethod:
          selectedInfo?.price && selectedInfo.price > 0 ? paymentMethod : null,
        ...
        { text: t("cancel") || "Annuler", style: "cancel" },
        {
          ...
          onPress: async () => {
            try {
              ...
              Alert.alert(
                t("successTitle") || "Succès",
                t("planDeactivated") || "Votre plan a été désactivé.",
              );
              onBack();
            } catch (e) {
              Alert.alert(
                t("error") || "Erreur",
                t("failedToSave") || "Échec de la mise à jour.",
              );
            }
          },
        },
    ...
    if (user?.uid) {
      try {
        ...
        if (!querySnapshot.empty) {
          ...
          if (appDoc.businessAddress)
            setBusinessAddress(appDoc.businessAddress);
          ...
          if (appDoc.businessLicense)
            setBusinessLicense(appDoc.businessLicense);
          ...
          if (appDoc.businessCategory) {
            ...
            if (
              predefinedCategories.includes(
                appDoc.businessCategory.toLowerCase(),
              )
            ) {
  ...
  const handleSubmitPaymentProof = async () => {
    ...
    try {
      ...
      if (!querySnapshot.empty) {
        ...
        Alert.alert(
          t("successTitle") || "Succès",
          t("vendorPaymentSent") || "Preuve de paiement envoyée avec succès.",
        );
        setPaymentProofForApproval(null);
      } else {
        Alert.alert(
          t("error") || "Erreur",
          t("adminVendorLoadError") || "Impossible de trouver votre demande.",
        );
      }
    } catch (e) {
      console.error(e);
      Alert.alert(
        t("error") || "Erreur",
        t("failedToSave") || "Échec de l'envoi.",
      );
    } finally {
        ...
        >
          <View
            style={[
              ...
              {
                backgroundColor: isDark ? "#2A2520" : "#FFF8F0",
                borderColor: "#F59E0B",
              },
            ]}
          >
          ...
          >
            ...
            >
              <Text
                style={[styles.statusCardLabel, { color: colors.textMuted }]}
              >
                {t("plan") || "Plan"}
              </Text>
              <Text style={[styles.statusCardValue, { color: accent }]}>
                {existingVendorData?.tier?.toUpperCase() || "—"}
              </Text>
            </View>
            <View
              style={{
            ...
            >
              <Text
                style={[styles.statusCardLabel, { color: colors.textMuted }]}
              >
                {t("businessName") || "Entreprise"}
              </Text>
              <Text
                style={[styles.statusCardValue, { color: colors.foreground }]}
              >
                {existingVendorData?.businessName || "—"}
              </Text>
            </View>
          </View>
          ...
          >
            ...
            >
              {t("vendorPending") || "En attente de confirmation"}
            </Text>
          </View>
  ...
  if (
    !isChangingPlan &&
    (existingStatus === "approved" || existingStatus === "active")
  ) {
    const currentTier = VENDOR_TIERS.find(
      (t) => t.id === existingVendorData?.tier,
    );
        ...
        <ScrollView contentContainerStyle={{ padding: 24, paddingBottom: 60 }}>
          <View
            style={[
              styles.statusIconCircle,
              {
                backgroundColor: isDark ? "#0D2520" : "#F0FFF8",
          ...
          >
            {isActive
              ? t("vendorPlanActiveDesc") ||
                "Votre plan vendeur est confirmé et actif."
            ...
            >
              <View style={styles.activePlanHeader}>
                <View>
                  ...
                  <Text
                    style={[
                      styles.activePlanName,
                      { color: colors.foreground },
                    ]}
                  >
                    {t(currentTier.name) || currentTier.name}
                  </Text>
                </View>
                <View
                  style={[
                ...
                >
                  <Text style={[styles.activePlanBadgeText, { color: accent }]}>
                    {currentTier.price === 0
                      ? t("free") || "Gratuit"
                      : `${currentTier.price} TND/${t("month") || "mois"}`}
                  </Text>
                </View>
              </View>
              <View
            ...
            >
              ...
              {hasPaymentProofUploaded ? (
                <>
                  ...
                  <View
                    style={[
                      styles.paymentProofWaiting,
                      {
                        backgroundColor: isDark ? "#1A1A10" : "#FFFBEB",
                ...
                <>
                  ...
                  <TouchableOpacity
                    style={[
                      styles.uploadBox,
                      {
                        ...
                        borderColor: paymentProofForApproval
                          ? "#10B981"
                          : colors.border,
                      },
                    ]}
                    onPress={async () => {
                      try {
                        ...
                        console.error("Error picking payment proof:", error);
                      }
                    }}
                  >
                    {paymentProofForApproval ? (
                      <Image
                        source={{ uri: paymentProofForApproval }}
                      ...
                      <View style={styles.uploadPlaceholder}>
                        ...
                        <Text
                          style={[styles.uploadText, { color: colors.textMuted }]}
                        >
                          {t("vendorUploadPaymentProof") ||
                  ...
                  <TouchableOpacity
                    style={[
                      ...
                      {
                        backgroundColor: paymentProofForApproval
                          ? "#10B981"
                          : colors.border,
                        opacity:
                          paymentProofForApproval && !uploadingPaymentProof
                            ? 1
                            : 0.5,
                      },
                    ]}
          ...
          {!isActive && currentTier && currentTier.price === 0 && (
            ...
            >
              ...
              >
                ...
                <Text
                  style={{ color: "#10B981", fontWeight: "700", fontSize: 15 }}
                >
        ...
        >
          <View
            style={[
              styles.statusIconCircle,
              {
                backgroundColor: isDark ? "#201515" : "#FFF5F5",
                borderColor: "#EF4444",
                marginTop: 20,
              },
            ]}
          ...
          <Text style={[styles.statusTitle, { color: colors.foreground }]}>
            {t("applicationRejected") || "Demande refusée"}
          </Text>
          <Text style={[styles.statusDesc, { color: colors.textMuted }]}>
      ...
      >
          ...
          <View>
            ...
            <TouchableOpacity
              style={[
                ...
                { borderColor: accent, marginBottom: 16 },
              ]}
              onPress={() => setShowComparison(!showComparison)}
            >
              <Text style={[styles.compareButtonText, { color: accent }]}>
                {showComparison
                  ? t("hideFeatures") || "Hide Features"
                    ...
                    <View style={styles.comparisonRow}>
                      <View
                        style={[
                          ...
                          styles.comparisonFeatureHeader,
                          { width: 120 },
                        ]}
                      >
                        <Text
                      ...
                      >
                        ...
                        {VENDOR_TIERS.map((tier) => {
                          ...
                          return (
                            <View
                              key={tier.id}
                              style={[styles.comparisonCell, { width: 100 }]}
                            >
              ...
              <TouchableOpacity
                ...
                style={[
                  styles.tierCard,
                  {
                    backgroundColor: isDark ? "#1C1C1E" : "#FFF",
                    borderColor:
                      selectedTier === tier.id ? accent : colors.border,
                  },
                ]}
              ...
              >
                {(tier.popular || tier.bestValue) && (
                  <View
                    style={[
                      styles.popularBadge,
                      { backgroundColor: tier.bestValue ? "#10B981" : accent },
                    ]}
                  >
                ...
                <View style={styles.tierHeader}>
                  <View>
                    <Text
                      style={[styles.tierName, { color: colors.foreground }]}
                    >
                ...
                <View style={styles.tierLimits}>
                  <View style={styles.limitItem}>
                    <Text
                      style={[styles.limitLabel, { color: colors.textMuted }]}
                    >
                      {t("featureProducts")}:
                    </Text>
                    <Text
                    ...
                    >
                      {typeof tier.limits.products === "number"
                        ? tier.limits.products
                        : t("featureProductsUnlimited")}
                    </Text>
                  </View>
                  <View style={styles.limitItem}>
                    <Text
                    ...
                    <Text
                      style={[styles.limitValue, { color: colors.foreground }]}
                    >
                      {typeof tier.limits.ordersPerMonth === "number"
        ...
        {step === 2 && (
          <View>
                        ...
                        backgroundColor:
                          accountType === type
                            ...
                            : isDark
                              ...
                              : "#FFF",
                        borderColor:
                          accountType === type ? accent : colors.border,
                      },
                    ]}
                    ...
                    <View style={styles.accountTypeContent}>
                      ...
                      <Text
                        style={[
                          ...
                          {
                            color:
                              accountType === type ? "#FFF" : colors.foreground,
                          },
                        ]}
                      >
                        {t(type)}
            ...
            <View style={styles.inputGroup}>
              ...
              >
                ...
                <TextInput
                  ...
                  placeholder={
                    t("businessNamePlaceholder") || "Enter your business name"
                  }
                        ...
                        backgroundColor:
                          businessCategory === cat
                            ...
                            : isDark
                              ...
                              : "#FFF",
                        borderColor:
                          businessCategory === cat ? accent : colors.border,
                      },
                    ]}
              ...
              <View style={styles.inputGroup}>
                ...
                >
                  <TextInput
                    ...
                    placeholder={
                      t("specifyCategoryPlaceholder") ||
                      "e.g., Baby products, Music instruments..."
                    }
                    placeholderTextColor={colors.textMuted}
                  />
                </View>
            ...
            <View style={styles.inputGroup}>
              ...
              >
                ...
                <TextInput
                  ...
                  placeholder={
                    t("addressPlaceholder") || "Enter your business address"
                  }
                  placeholderTextColor={colors.textMuted}
                  multiline
                />
            ...
            <View style={styles.inputGroup}>
              ...
              >
                <TextInput
                  ...
                  placeholder={
                    t("vendorDescriptionPlaceholder") ||
            ...
            >
              {t("vendorSocialMedia") || "Social Media"} (
              {t("optional") || "Optional"})
            </Text>
          ...
          <View>
            ...
            >
              ...
              <Text style={[styles.kycDesc, { color: colors.textMuted }]}>
                {t("storeVerificationDesc") ||
                  "Please upload photos of your store or business licenses if available."}
              </Text>
            </View>
            ...
            <View style={styles.documentSection}>
              <Text style={[styles.inputLabel, { color: colors.textMuted }]}>
                {t("storeFront") || "Store Front Photo"} (
                {t("optional") || "Optional"})
              </Text>
              <TouchableOpacity
                style={[
            ...
            {(() => {
              const selectedInfo = VENDOR_TIERS.find(
                (t) => t.id === selectedTier,
              );
                  ...
                  <View style={styles.documentSection}>
                    ...
                    >
                      ...
                      <Text style={{ color: accent, fontWeight: "700" }}>
                        {selectedInfo.price} TND / {t("month") || "mois"}
                      </Text>
                    </Text>
                    ...
                    <View style={styles.paymentMethods}>
                      <TouchableOpacity
                        style={[
                          styles.paymentMethodBtn,
                          {
                            backgroundColor: isDark ? "#1C1C1E" : "#FFF",
                            borderColor:
                              paymentMethod === "bank" ? accent : colors.border,
                          },
                        ]}
                        onPress={() => {
                      ...
                      >
                        <Building2
                          size={24}
                          color={
                            paymentMethod === "bank" ? accent : colors.textMuted
                          }
                        />
                        <Text
                          style={[
                      ...
                      <TouchableOpacity
                        style={[
                          styles.paymentMethodBtn,
                          {
                            backgroundColor: isDark ? "#1C1C1E" : "#FFF",
                            borderColor:
                              paymentMethod === "post" ? accent : colors.border,
                          },
                        ]}
                        onPress={() => {
                      ...
                      >
                        <Mail
                          size={24}
                          color={
                            paymentMethod === "post" ? accent : colors.textMuted
                          }
                        />
                        <Text
                          style={[
                    ...
                    {paymentMethod && (
                      <View style={{ marginBottom: 16 }}>
                        <TouchableOpacity
                          style={[
                            styles.paymentDetailsToggle,
                            {
                              backgroundColor: isDark ? "#1C1C1E" : "#F8F8FB",
                          ...
                          >
                            ...
                            >
                              {paymentMethod === "bank"
                                ...
                                : t("viewPostDetails") ||
                                  "Voir les coordonnées du mandat"}
                            </Text>
                          </View>
                          {showPaymentDetails ? (
                            <ArrowUp size={18} color={accent} />
                          ) : (
                            <ArrowDown size={18} color={accent} />
                          )}
                        </TouchableOpacity>
                          ...
                          >
                            {paymentMethod === "bank" ? (
                              <>
                                ...
                                >
                                  {t("paymentRef") ||
                                    "Mentionnez votre nom et email en référence du virement."}
                                </Text>
                              </>
                            ) : (
                              <>
                                ...
                                >
                                  📮{" "}
                                  {t("postTransferDetails") ||
                                    "Coordonnées du mandat postal"}
                                </Text>
                                <View style={styles.paymentDetailRow}>
                                  <Text
                                    style={[
                                ...
                                >
                                  {t("paymentRef") ||
                                    "Mentionnez votre nom et email en référence du mandat."}
                                </Text>
                              </>
                            )}
                          </View>
                    ...
                    <TouchableOpacity
                      style={[
                        ...
                        {
                          backgroundColor: isDark ? "#1C1C1E" : "#FFF",
                          borderColor: paymentProof ? accent : colors.border,
                        },
                      ]}
                    ...
                    >
                      {paymentProof ? (
                        ...
                        <View style={styles.uploadPlaceholder}>
                          ...
                          >
                            {t("uploadReceiptHint") ||
                              "Téléchargez une photo de votre reçu de paiement"}
                          </Text>
                        </View>
                      )}
                    </TouchableOpacity>
            ...
            >
              ...
              >
                ...
                <Text style={styles.contractDetails}>
                  Plateforme de vente en ligne - Tunisie
                </Text>
                <Text style={styles.contractDetails}>
                  Capital : 1 000 DT | RC : 1927947DAM000
                </Text>

                <Text style={styles.articleTitle}>PRÉAMBULE</Text>
                <Text style={styles.articleText}>
                  Le présent contrat de partenariat (ci-après « le Contrat »)
                  est conclu entre : BEY3A, plateforme de vente en ligne opérant
                  sur le territoire tunisien, représentée par son gérant
                  (ci-après « la Plateforme »), D'UNE PART, ET LE VENDEUR, tel
                  que décrit dans le formulaire d'inscription (ci-après « le
                ...
                <Text style={styles.articleText}>
                  ...
                  lesquelles le Vendeur propose et vend ses produits/services
                  sur la plateforme BEY3A. La Plateforme agit en qualité
                  d'intermédiaire de vente et de paiement pour le compte du
                  Vendeur.
                </Text>
                ...
                <Text style={styles.articleText}>
                  2.1. Le Vendeur s'engage à deposer des produits conformes aux
                  descriptions fournies, en bon état et conformes aux normes
                  tunisiennes en vigueur.{"\n"}
                  2.2. Le Vendeur doit fournir toutes les informations
                  obligatoires relatives aux produits (composition, origine,
                  prix, caractéristiques, etc.).{"\n"}
                  2.3. Le Vendeur s'engage à maintenir un stock suffisant et à
                  informer immédiatement BEY3A de toute rupture de stock.{"\n"}
                  2.4. Le Vendeur doit respecter les délais de livraison
                  convenus et assurer la qualité du service après-vente.
                </Text>

                <Text style={styles.articleTitle}>ARTICLE 3 : COMMISSIONS</Text>
                <Text style={styles.articleText}>
                  3.1. BEY3A perçoit une commission de 24% TTC (toutes taxes
                  comprises) sur chaque vente réalisée par l'intermédiaire de la
                  Plateforme.{"\n"}
                  3.2. Cette commission est prelevee sur le prix de vente hors
                  taxes et inclut toutes les taxes applicables (TVA 19%).{"\n"}
                  3.3. Le Vendeur recoit donc 76% du prix de vente hors taxes
                ...
                <Text style={styles.articleText}>
                  4.1. Les paiements au Vendeur sont effectués mensuellement,
                  dans un délai de 30 jours suivant la fin de chaque mois civil.
                  {"\n"}
                  4.2. Le paiement est effectué par virement bancaire sur le
                  compte fourni par le Vendeur lors de son inscription.{"\n"}
                  4.3. En cas de litige sur une commande, le paiement
                  correspondant peut être suspendu jusqu'à résolution dudit
                  litige.{"\n"}
                  4.4. Le montant minimum de paiement est de 100 DT. Les
                  montants inférieurs seront reportés au mois suivant.
                </Text>

                <Text style={styles.articleTitle}>
                  ARTICLE 5 : CONDITIONS D'INSCRIPTION ET DE CRÉATION DES
                  ANNONCES
                </Text>
                <Text style={styles.articleText}>
                  5.1. Le Vendeur doit compléter le formulaire d'inscription et
                  fournir les documents requis.{"\n"}
                  5.2. Chaque produit doit faire l'objet d'une annonce complète
                  incluant photos, description, prix, tailles disponibles et
                  conditions de livraison.{"\n"}
                  5.3. BEY3A se réserve le droit de refuser ou de supprimer
                  toute annonce non conforme à ses règles de publication.
                </Text>
                ...
                <Text style={styles.articleText}>
                  6.1. Le Vendeur est seul responsable de la conformité de ses
                  produits aux normes légales et réglementaires en vigueur en
                  Tunisie.{"\n"}
                  6.2. BEY3A ne peut être tenue responsable des vices cachés,
                  défauts de conformité ou autres problèmes relatifs aux
                  produits vendus.{"\n"}
                  6.3. Le Vendeur s'engage à garantir BEY3A contre toute
                  réclamation, action ou dommage résultant de ses produits.
                </Text>
                ...
                <Text style={styles.articleText}>
                  Le Vendeur garantit être propriétaire des droits de propriété
                  intellectuelle. Il autorise BEY3A à utiliser ces éléments.
                </Text>
                ...
                <Text style={styles.articleText}>
                  8.1. Le présent Contrat est conclu pour une durée de 10 (dix)
                  ans à compter de la date de sa signature par les deux parties.
                  {"\n"}
                  8.2. Le Contrat est tacitement renouvelé pour des périodes
                  successives de 10 ans, sauf notification contraire par l'une
                  des parties, par lettre recommandée avec accusé de réception,
                  au moins 3 (trois) mois avant l'expiration de la période en
                  cours.
                </Text>

                <Text style={styles.articleTitle}>ARTICLE 9 : RÉSILIATION</Text>
                <Text style={styles.articleText}>
                  9.1. Chaque partie peut résilier le Contrat de plein droit en
                  cas de manquement grave de l'autre partie à ses obligations,
                  après mise en demeure restée sans effet pendant 30 jours.
                  {"\n"}
                  9.2. AZOOLO MARKETPLACE peut résilier immédiatement le Contrat
                  en cas de comportement frauduleux, de violation des lois
                  applicables ou de non-respect des règles de la Plateforme.
                </Text>
                ...
                <Text style={styles.articleText}>
                  ...
                  informations échangées dans le cadre de l'exécution du présent
                  Contrat.
                </Text>
                ...
                <Text style={styles.articleText}>
                  11.1. Le présent Contrat est régi par le droit tunisien.{"\n"}
                  11.2. Tout litige relatif à l'interprétation ou à l'exécution
                  du présent Contrat sera de la compétence exclusive des
                  tribunaux de Tunis (Tunisie).
                </Text>
                ...
                <Text style={styles.articleText}>
                  12.1. Le fait pour l'une des parties de ne pas se prévaloir
                  d'une disposition du Contrat ne vaut pas renonciation à cette
                  disposition.{"\n"}
                  12.2. Si une disposition du Contrat est déclarée nulle ou non
                  exécutoire, les autres dispositions demeurent en vigueur.
                  {"\n"}
                  12.3. AZOOLO MARKETPLACE se réserve le droit de modifier les
                  présentes conditions générales.
                </Text>

                <Text style={styles.articleTitle}>
                  ARTICLE 13 : ACCEPTATION EXPRESSE
                </Text>
                <Text style={styles.articleText}>
                  Le Vendeur reconnaît avoir lu, compris et accepté sans réserve
                  l'ensemble des dispositions du présent Contrat de dépôt-vente.
                  Il reconnaît que l'acceptation du présent Contrat constitue un
                  élément essentiel sans lequel BEY3A ne permettrait pas
                  l'activation de son compte vendeur.
                </Text>
            ...
            >
              ...
              <Text
                style={[
                  ...
                  {
                    color: hasScrolledToEnd
                      ? colors.foreground
                      : colors.textMuted,
                  },
                ]}
              >
                Je reconnais avoir lu et accepté les 13 articles du présent
                Contrat de dépôt-vente et ses conditions générales
              </Text>
            </TouchableOpacity>

            {!hasScrolledToEnd && (
              <Text style={[styles.warningText, { color: "#FF6B6B" }]}>
                * Vous devez lire le contrat en entier avant de pouvoir
                l'accepter
              </Text>
            )}
          </View>
