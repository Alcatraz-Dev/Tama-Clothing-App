...
interface HomeScreenProps {
    ...
    onCategoryPress: (catId: string, catName?: string) => void;
...
export default function HomeScreen({
    ...
    const fetchData = async () => {
        try {
            ...
            setCategories(allCats.filter((c: any) => !c.parentId));
        ...
        <View style={[styles.container, { backgroundColor }]}>
            <Animated.View style={[styles.modernHeader, {
                ...
                <Animated.View style={[StyleSheet.absoluteFill, { opacity: headerOpacity }]}>
                    <BlurView intensity={80} style={StyleSheet.absoluteFill} tint={theme} blurTarget={blurTargetRef} blurMethod="dimezisBlurView" />
            ...
            <BlurTargetView ref={blurTargetRef} style={{ flex: 1 }}>
                ...
                >
                                ...
                                >
                                    <View style={styles.liveAvatarContainer}>
                                        ...
                                        <Animatable.View animation="bounceIn" style={[styles.liveBadge, { backgroundColor: errorColor, borderColor: theme === 'dark' ? '#121218' : '#FFF' }]}>
                                            <Text variant="caption" style={styles.liveBadgeText}>{t('enDirect')}</Text>
                                        </Animatable.View>
                                    </View>

                                    <View style={{ flex: 1, justifyContent: 'center' }}>
                                        <Text variant="body" style={{ letterSpacing: -0.5, marginBottom: 4, fontSize: 13, fontWeight: '800' }} numberOfLines={1}>
                                            {((bestCollab ? getName(bestCollab.name, language) : (bestLiveSession.hostName || t('broadcaster')))).toUpperCase()}
                                        </Text>
                                    ...
                                    >
                                        <View style={{ position: 'relative', alignItems: 'center' }}>
                                            ...
                                            <Animatable.View animation="bounceIn" style={[styles.liveNowBadge, { backgroundColor: errorColor, borderColor: backgroundColor }]}>
                                                <Text style={styles.liveNowBadgeText}>{t('enDirect')}</Text>
                                            </Animatable.View>
                                        </View>
                    ...
                    <View style={styles.modernSection}>
                        <View style={styles.sectionHeader}>
                            <View>
                                <Text variant="heading" style={{ letterSpacing: 4, fontSize: 13 }}>{t('collections')}</Text>
                            ...
                            <View>
                                ...
                                <View style={{ flexDirection: 'row', gap: 12 }}>
                                    {categories.map((cat: any) => (
                                        <TouchableOpacity key={cat.id} style={{ alignItems: 'center' }} onPress={() => onCategoryPress(cat.id, getName(cat.name, language))}>
                            ...
                            <View>
                                ...
                                <View style={{ flexDirection: 'row', gap: 12 }}>
                                    {brands.map((brand: any) => (
                                        <TouchableOpacity key={brand.id} style={{ alignItems: 'center' }} onPress={() => { setFilterBrand(brand.id); onNavigate('Shop'); }}>
                                            <View style={[styles.modernCatCard, { backgroundColor: theme === 'dark' ? '#17171F' : '#F9F9F9', borderColor: borderSecondary }]}>
                                                <Image source={{ uri: brand.image || 'https://images.unsplash.com/photo-1441984904996-e0b6ba687e04?w=400' }} style={styles.catBgImage} />
                                            </View>
                                            <Text variant="caption" style={{ marginTop: 8, letterSpacing: 1, fontSize: 9 }}>{brand.name ? (typeof brand.name === 'string' ? brand.name : brand.name[language])?.toUpperCase() : ''}</Text>
                                        </TouchableOpacity>
                                    ))}
                                </View>
                            </View>
