...
export default function AdminKYCScreen({
    ...
    const unsubscribe = onSnapshot(
      ...
      async (snapshot) => {
        ...
        enriched.sort((a: any, b: any) => {
          const dateA = a.kycData?.submittedAt
            ? new Date(a.kycData.submittedAt).getTime()
            : 0;
          const dateB = b.kycData?.submittedAt
            ? new Date(b.kycData.submittedAt).getTime()
            : 0;
          return dateB - dateA;
        });
  ...
  const handleAction = async (status: "approved" | "rejected") => {
    ...
    try {
      ...
      await addDoc(collection(db, "kyc_history"), {
        ...
        rejectionReason:
          status === "rejected"
            ? "Incomplete or invalid verification info"
            : null,
      });

      Alert.alert(
        status === "approved" ? "Approved" : "Rejected",
        `Verification has been ${status}.`,
      );
    ...
    >
      ...
      >
        ...
        <View style={{ flex: 1, justifyContent: "center" }}>
          ...
          <Text
            style={[styles.email, { color: isDark ? "#AAA" : "#666" }]}
            numberOfLines={1}
          >
            {item.email || "No Email"}
          </Text>
        ...
        <View style={{ alignItems: "flex-end" }}>
          <Text
            style={{ fontSize: 10, color: colors.textMuted, fontWeight: "700" }}
          >
            {item.kycData?.submittedAt
              ? new Date(item.kycData.submittedAt).toLocaleDateString()
              : "N/A"}
          </Text>
          <ChevronLeft
            size={16}
          ...
          <View style={{ flex: 1, justifyContent: "center" }}>
            ...
            >
              ...
              <Text
                style={{
                  fontSize: 11,
                  color: isApproved ? "#34C759" : "#FF3B30",
      ...
      >
        ...
        >
          <View
            style={[
              styles.modalHeader,
              { borderBottomColor: colors.border, marginTop: 40 },
            ]}
          >
            <Text style={[styles.modalTitle, { color: colors.foreground }]}>
              {t("faceVerification").toUpperCase()}
            </Text>
            <TouchableOpacity
              ...
              <View style={{ marginTop: 30, marginBottom: 20 }}>
                ...
                {userHistory.map((h, idx) => (
                  ...
                  >
                      ...
                      >
                        ...
                        <Text
                          style={[
                            ...
                            {
                              color:
                                h.status === "approved" ? "#34C759" : "#FF3B30",
                            },
                          ]}
                        >
                    ...
                    >
                      Role: {h.requestedRole?.toUpperCase()}
                    </Text>
                    {h.status === "rejected" && (
                      <Text
                        style={{
  ...
  return (
    <View style={[styles.container, { backgroundColor: colors.background }]}>
      <AdminHeader title={t("faceVerification")} onBack={onBack} />
