...
function getString(val: any): string {
    ...
    return val.fr || val.en || val['ar-tn'] || val.ar || Object.values(val)[0] || '';
}

// ─── Sub-components ───────────────────────────────────────────────────────────
function ProductListItem({ item, onEdit, onDuplicate, onDelete, colors, theme, language, categories = [], t }: any) {
    const isDark = theme === 'dark';
    const name = getString(item.name);

    // Find the category object to show parent > child if applicable
    const catObj = categories.find((c: any) => c.id === item.categoryId);
    const parent = catObj?.parentId ? categories.find((c: any) => c.id === catObj.parentId) : null;
    const category = parent
        ? `${getString(parent.name)} > ${getString(catObj.name)}`
        : (catObj ? getString(catObj.name) : getString(item.category));
            ...
            <View style={sc.thumbWrapper}>
                ...
                {item.status === 'sold_out' && (
                    <View style={sc.soldOutBadge}>
                        <Text style={sc.soldOutText}>{t('soldOut')}</Text>
                    </View>
                )}
            </View>
...
export default function AdminProductsScreen({ onBack, t, profileData, language = 'fr' }: any) {
                    ...
                    onPress: async () => {
                        ...
                        try {
                            ...
                            const data = {
                                ...cleanData,
                                name: {
                                    fr: (p.name?.fr || getString(p.name)) + ` (${t('copy')})`,
    ...
    return (
        <View style={[sc.root, { backgroundColor: colors.background }]}>
            ...
            {/* Add / Edit Modal */}
            <Modal visible={modalVisible} animationType="slide" presentationStyle="formSheet" transparent={false}>
                        ...
                        >
                            ...
                            <ScrollView horizontal showsHorizontalScrollIndicator={false} style={{ marginBottom: 20 }}>
                                <TouchableOpacity onPress={handlePickImage} style={[sc.addImageBtn, { backgroundColor: theme === 'dark' ? '#121218' : '#FFF', borderColor: colors.border }]}>
                                    ...
                                    <Text style={[sc.addImageText, { color: colors.textMuted }]}>{t('add')}</Text>
                                </TouchableOpacity>
                                {images.map((img, i) => (
                            ...
                            {!videoUrl ? (
                                ...
                                >
                                    ...
                                    <Text style={[sc.videoPlaceholderText, { color: colors.textMuted }]}>{t('tapToUploadVideo')}</Text>
                                </TouchableOpacity>
                            ) : (
                            ...
                            <TextInput
                                style={[sc.input, { backgroundColor: theme === 'dark' ? '#17171F' : '#FFF', color: colors.foreground, borderColor: colors.border }]}
                                value={zone}
                                onChangeText={setZone}
                                placeholder="e.g. Tunis, Sousse, Global"
                                placeholderTextColor={colors.textMuted}
                            />
