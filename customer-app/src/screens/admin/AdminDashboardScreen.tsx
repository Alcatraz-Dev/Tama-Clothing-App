...
function StatCard({ label, value, icon: Icon, color }: StatCardProps) {
  ...
  return (
    ...
    >
      ...
      <Text
        style={[sc.statValue, { color: colors.foreground }]}
        numberOfLines={1}
        adjustsFontSizeToFit
      >
        {value}
      </Text>
      <Text style={[sc.statLabel, { color: colors.textMuted }]}>
        {label.toUpperCase()}
      </Text>
      {/* Accent dot */}
...
export default function AdminDashboardScreen({
  ...
  const fetchDashboardData = async () => {
    try {
      ...
      const myBrandId =
        profileData?.brandId ||
        profileData?.vendorOwnerId ||
        (profileData?.role === "vendor" ? profileData?.uid : null);
      ...
      ordersData.forEach((o: any) => {
        if (isBrandOwner && myBrandId) {
          const brandItems = (o.items || []).filter(
            (i: any) => i.brandId === myBrandId,
          );
      ...
      if (isBrandOwner && myBrandId) {
        orderCount = ordersData.filter((o: any) =>
          (o.items || []).some((i: any) => i.brandId === myBrandId),
        ).length;
      }
      ...
      if (isBrandOwner && myBrandId) {
        productsCount = productsSnap.docs.filter(
          (d) => d.data().brandId === myBrandId,
        ).length;
      }
      ...
      if (prizeStatsSnap.exists()) {
        ...
        if (!isBrandOwner) {
          // Only add to global admin view, usually
      ...
      let validOrders = ordersData.filter(
        (o: any) => o.createdAt && typeof o.createdAt.seconds === "number",
      );
      if (isBrandOwner && myBrandId) {
        validOrders = validOrders.filter((o: any) =>
          (o.items || []).some((i: any) => i.brandId === myBrandId),
        );
      }
          ...
          {loading ? (
            ...
            <>
              {(profileData?.role === "brand_owner" ||
                ...
                >
                  <View style={sc.brandBannerContent}>
                    <View>
                      <Text
                        style={[sc.brandName, { color: colors.foreground }]}
                      >
                        {profileData.brandName?.toUpperCase() ||
                          profileData.vendorData?.businessName?.toUpperCase() ||
                          t("brandOwner")}
                      </Text>
                      <View
                        style={{
                      ...
                      >
                        <Text
                          style={[sc.brandRole, { color: colors.textMuted }]}
                        >
                        ...
                        >
                          ...
                          >
                            {(
                              (profileData.vendorPlan as string) ||
                              profileData.vendorData?.tier ||
                              "starter"
                            ).toUpperCase()}
                          </Text>
                        </View>
                    ...
                    >
                      <Text style={[sc.badgeRoleText, { color: "#34C759" }]}>
                        VERIFIED
                      </Text>
                    </View>
                  </View>
              ...
              <View style={sc.statsGrid}>
                <StatCard
                  label={t("totalSales")}
                  value={`${stats.sales.toFixed(2)} TND`}
                  icon={ShoppingCart}
                  color="#34C759"
                />
                <StatCard
              ...
              {profileData?.role === "admin" && (
                <>
                  ...
                  >
                    <View
                      style={[
                        sc.statIconBox,
                        {
                          backgroundColor: "#34C759" + (isDark ? "20" : "14"),
                          marginBottom: 0,
                        },
                      ]}
                    >
                    ...
                    <View style={{ flex: 1, marginLeft: 15 }}>
                      ...
                      >
                        {t("verifyIdentityDesc").toUpperCase()}
                      </Text>
                    </View>
                    <ChevronLeft
                      size={18}
                  ...
                  <TouchableOpacity
                    onPress={() =>
                      onNavigate && onNavigate("AdminNotreSelection")
                    }
                    style={[
                      sc.quickActionCard,
                      {
                  ...
                  >
                    <View
                      style={[
                        sc.statIconBox,
                        {
                          backgroundColor: "#10B981" + (isDark ? "20" : "14"),
                          marginBottom: 0,
                        },
                      ]}
                    >
                    ...
                    <View style={{ flex: 1, marginLeft: 15 }}>
                      ...
                      <Text
                        style={[sc.quickActionSub, { color: colors.textMuted }]}
                      >
                        {t("ourSelectionDesc").toUpperCase()}
                      </Text>
                    </View>
                    <ChevronLeft
                      size={18}
                  ...
                  >
                    <View
                      style={[
                        sc.statIconBox,
                        {
                          backgroundColor: "#FFD700" + (isDark ? "20" : "14"),
                          marginBottom: 0,
                        },
                      ]}
                    >
              ...
              {recentOrders.length > 0 && (
                <Text
                  style={[
                    sc.sectionTitle,
                    { color: colors.foreground, marginTop: 25 },
                  ]}
                >
                  {t("recentOrders").toUpperCase()}
                </Text>
              )}
