...
export default function CollaborationDetailScreen({
  ...
  const [liveSession, setLiveSession] = React.useState<LiveSession | null>(
    null,
  );
  const [showBadge, setShowBadge] = React.useState(false);
  const [localFollowersCount, setLocalFollowersCount] = React.useState(collab.followersCount || 0);
  ...
  // Handler for follow/unfollow with optimistic UI update
  const handleToggleFollow = () => {
    // Optimistic update: increment or decrement local count
    ...
    if (toggleFollowCollab) {
      toggleFollowCollab(collab.id);
    }
  };
  ...
  const getTabs = () => {
    const base = [
      { id: "about", label: t("about") || tr("À PROPOS", "حول", "ABOUT") },
    ];
    ...
    if (collab.type === "Brand") {
      if (products.length > 0) {
        base.push({
          id: "products",
          label: t("products") || tr("PRODUITS", "السلع", "PRODUCTS"),
        });
      }
      ...
      if (hasSocials) {
        base.push({
          id: "socials",
          label: t("socialMedia") || tr("SOCIALS", "سوشيال ميديا", "SOCIALS"),
        });
      }
      ...
      if (hasInfo) {
        base.push({
          id: "info",
          label: t("info") || tr("INFOS", "معلومات", "INFO"),
        });
      }
    }
    ...
    if (collab.type !== "Brand") {
      base.push({
        id: "reviews",
        label: t("reviews") || tr("AVIS", "لي ريفيو", "REVIEWS"),
      });
    }
    return base;
  };
      ...
      try {
        ...
        if (pList.length > 0) {
          const total = pList.reduce(
            (acc, p: any) => acc + (parseFloat(p.rating) || 5.0),
            0,
          );
  ...
  const fetchCollabReviews = React.useCallback(async () => {
    if (collab.type !== "Brand") {
      ...
      try {
        ...
        if (rList.length > 0) {
          const total = rList.reduce(
            (acc, r: any) => acc + (parseFloat(r.rating) || 5.0),
            0,
          );
        ...
        { text: t("cancel"), style: "cancel" },
        {
          ...
          onPress: async () => {
            try {
              ...
              Alert.alert(
                t("success") || "SUCCESS",
                t("reviewDeleted") || "Review deleted",
              );
              fetchCollabReviews();
            } catch (err) {
  ...
  const handleLiveAction = () => {
    if (!auth.currentUser) {
      Alert.alert(
        t("error"),
        t("pleaseLoginToJoinLive") || "Please login to join live",
      );
      return;
    }
    ...
    console.log(
      "🎬 CollabDetail Live Action - isLive:",
      isLive,
      "isOwner:",
      isOwner,
      "isAdmin:",
    ...
    onJoinLive({
      channelId:
        isLive && liveSession?.channelId ? liveSession.channelId : collab.id,
      isHost: isHost,
      userName:
        profileData?.fullName || auth.currentUser.displayName || "Guest",
      userId: auth.currentUser.uid,
  ...
  const SocialButton = ({
    ...
    return (
      ...
      >
        ...
        <Text
          style={[styles.socialButtonText, { color: isDark ? "#FFF" : "#000" }]}
        >
          {label}
        </Text>
      </TouchableOpacity>
              ...
              <>
                <Text
                  style={[
                    ...
                    { color: isDark ? "#FFF" : "#000", marginTop: 32 },
                  ]}
                >
                  {t("socialMedia")}
                </Text>
                <View style={styles.socialGrid}>
                  <SocialButton
                    icon={Instagram}
                    label="Instagram"
                    url={collab.instagramUrl}
                  />
                  <SocialButton
                    icon={Facebook}
                    label="Facebook"
                    url={collab.facebookUrl}
                  />
                  <SocialButton
                    icon={Twitter}
                    label="Twitter"
                    url={collab.twitterUrl}
                  />
                  <SocialButton
                    icon={Linkedin}
                    label="LinkedIn"
                    url={collab.linkedinUrl}
                  />
                  <SocialButton
                    icon={Music}
                    label="TikTok"
                    url={collab.tiktokUrl}
                  />
                </View>
              </>
            )}
              ...
              <View>
                ...
                <View style={{ paddingHorizontal: 20 }}>
                  <TouchableOpacity
                    ...
                    onPress={() =>
                      collab.brandId && onNavigateToShop(collab.brandId)
                    }
                  >
                    <Text
                      style={[
            ...
            <View style={{ gap: 12 }}>
              {collab.websiteUrl && (
                ...
                >
                  ...
                  >
                    {collab.websiteUrl.replace("https://", "").toUpperCase()}
                  </Text>
                </TouchableOpacity>
              )}
                ...
                <>
                  <Text
                    style={[
                      ...
                      {
                        color: isDark ? "#FFF" : "#000",
                        marginTop: collab.websiteUrl ? 15 : 0,
                      },
                    ]}
                  >
                    {t("socialMedia")}
                  ...
                  >
                    ...
                    >
                      <View style={{ flex: 1 }}>
                        ...
                        <View
                          style={{ flexDirection: "row", gap: 2, marginTop: 4 }}
                        >
                          {[1, 2, 3, 4, 5].map((s) => (
                            <Star
                              key={s}
                              size={10}
                              color={
                                s <= rev.rating ? "#FFD700" : colors.textMuted
                              }
  ...
  return (
    <View style={{ flex: 1, backgroundColor: isDark ? "#000" : "#FFF" }}>
      ...
      <View style={[styles.fixedHeaderButtons, { top: insets.top + 10 }]}>
        ...
        <Animated.View
          style={[styles.compactHeaderContent, headerOpacityStyle]}
        >
          ...
          <View>
            ...
            <View style={styles.compactBadgeRow}>
              ...
              <Text style={[styles.compactBadgeText, { color: badgeColor }]}>
                {t("officialPartner")}
              </Text>
            </View>
          </View>
        ...
        >
          ...
          <View style={styles.profileHeader}>
            ...
            <View style={styles.titleSection}>
              <Text
                style={[styles.brandName, { color: isDark ? "#FFF" : "#000" }]}
              >
                {collab.name}
              </Text>
              <View
                ...
                <View style={styles.liveSessionBanner}>
                  <View style={styles.liveSessionBadge}>
                    <Animatable.View
                      ...
                      style={styles.pulseDot}
                    />
                    <Text style={styles.liveSessionBadgeText}>
                      {t("enDirect") || "EN DIRECT"}
                    </Text>
                  </View>
              ...
              >
                ...
                {collab.type === "Brand" && collab.brandId && (
                  <TouchableOpacity
                    style={[
                      styles.primaryAction,
                      { backgroundColor: isDark ? "#FFF" : "#000", flex: 1 },
                    ]}
                    onPress={() => onNavigateToShop(collab.brandId!)}
                  >
                    <Text
          ...
          <View style={styles.statsBar}>
            ...
            <View style={styles.statItem}>
              ...
              >
                {localFollowersCount
                  ? Math.max(0, localFollowersCount) >= 1000
                    ? `${(Math.max(0, localFollowersCount) / 1000).toFixed(1)}k`
                    : Math.max(0, localFollowersCount)
                  : "0"}
              </Text>
              <View
                style={{ flexDirection: "row", alignItems: "center", gap: 2 }}
            ...
            {collab.type !== "Brand" && (
              <>
                ...
                <View style={styles.statItem}>
                  ...
                  >
                    {collab.worksCount
                      ? Math.max(0, collab.worksCount) >= 1000
                        ? `${(Math.max(0, collab.worksCount) / 1000).toFixed(1)}k`
                        : Math.max(0, collab.worksCount)
                      : "0"}
                  </Text>
                  <View
                    style={{
            ...
            <FlatList
              ...
              renderItem={() => (
                <View>
                  ...
                  <View
                    style={{ flexDirection: "row", gap: 15, marginBottom: 30 }}
                  >
                    {[1, 2, 3, 4, 5].map((s) => (
                      <TouchableOpacity
                        key={s}
                      ...
                      >
                        <Star
                          size={32}
                          color={s <= userRating ? "#FFD700" : colors.textMuted}
