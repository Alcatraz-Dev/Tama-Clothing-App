...
const MiniFlashTimer = ({ endTime }: { endTime: string }) => {
    ...
    return (
        <View style={[styles.miniTimer, { backgroundColor: timerColor, shadowColor: timerColor }]}>
            <Clock size={10} color={timerTextColor} strokeWidth={2.5} />
            <Text variant="caption" style={{ color: timerTextColor, fontWeight: '900', fontSize: 10 }}>
...
export default function ProductCard({
                    ...
                    <View style={styles.heroBottomInfo}>
                        {product.status === 'sold_out' && (
                            <View style={styles.soldOutBadge}>
                                ...
                                <Text variant="caption" style={styles.soldOutText}>{translate('soldOut').toUpperCase()}</Text>
                            </View>
                        )}
                        <Text variant="caption" style={styles.heroBrandName} numberOfLines={1}>{product.brandName || 'TAMA'}</Text>
                        <Text variant="title" style={styles.heroProductName} numberOfLines={2}>{getName(product.name)}</Text>
                        ...
                        <View style={styles.heroRatingRow}>
                            ...
                            <Text variant="caption" style={{ color: '#FFFFFF', fontWeight: '800' }}>
                                {product.rating || '5.0'}  <Text variant="caption" style={{ color: 'rgba(255,255,255,0.6)' }}>{product.reviewsCount !== undefined ? product.reviewsCount : (product.reviews?.length || 0)} {translate('reviews').toUpperCase()}</Text>
                            </Text>
                        </View>
    ...
    if (horizontal) {
        return (
            <TouchableOpacity
                style={[styles.horizontalCard, { backgroundColor: bgColor, borderColor }]}
                onPress={onPress}
                activeOpacity={0.9}
            >
                ...
                <View style={styles.horizontalInfo}>
                    <View>
                        <Text variant="caption" style={{ color: textMutedColor, letterSpacing: 0.5 }}>
                            {String(getName(product.brandName) || 'TAMA').toUpperCase()}
                        </Text>
                        <Text variant="subtitle" style={{ color: fgColor }} numberOfLines={1}>
                    ...
                    {showRating && (
                        <View style={styles.horizontalRating}>
                            ...
                            <Text variant="caption" style={{ color: fgColor }}>{product.rating || '5.0'}</Text>
                        </View>
                    )}
                </View>
                <View style={styles.horizontalActions}>
                    {onToggleWishlist && (
                        <Button
                            ...
                            style={[styles.actionCircle, { backgroundColor: isWishlisted ? redColor + '15' : mutedColor }]}
                            onPress={onToggleWishlist}
                        />
                    )}
                    {onAddToCart && (
                ...
                {product.status === 'sold_out' && (
                    <View style={styles.soldOutOverlayHorizontal}>
                        <Text variant="caption" style={styles.soldOutTextHorizontal}>{translate('soldOut').toUpperCase()}</Text>
                    </View>
                )}
            </TouchableOpacity>
        );
    ...
    return (
        <TouchableOpacity
            style={[styles.modernProductCard, { backgroundColor: bgColor, width: customWidth || (SCREEN_WIDTH - 48) / 2 }]}
            onPress={onPress}
            activeOpacity={0.9}
        >
                ...
                <View style={styles.modernCardContent}>
                    ...
                    {product.status === 'sold_out' && (
                        <View style={styles.soldOutBadgeModern}>
                            <View style={styles.soldOutDot} />
                            <Text variant="caption" style={styles.soldOutTextModern}>{translate('soldOut').toUpperCase()}</Text>
                        </View>
                    )}
                    <Text variant="caption" style={styles.modernBrandName} numberOfLines={1}>
                        {String(getName(product.brandName) || 'TAMA').toUpperCase()}
                    </Text>
                    <Text variant="subtitle" style={styles.modernProductName} numberOfLines={2}>
                        {getName(product.name)}
                    </Text>
                    <View style={styles.modernPriceRow}>
                        <Text variant="body" style={{ color: '#FFFFFF', fontWeight: '900' }}>
                    ...
                    {showRating && (
                        <View style={styles.modernRatingRow}>
                            <Star size={12} color="#FFFFFF" fill="#FFFFFF" />
                            <Text variant="caption" style={{ color: '#FFFFFF', fontWeight: '800' }}>
                                {product.rating || '5.0'}   <Text variant="caption" style={{ opacity: 0.6 }}>{product.reviewsCount !== undefined ? product.reviewsCount : (product.reviews?.length || 0)} {translate('reviews').toUpperCase()}</Text>
                            </Text>
                        </View>
                    )}
