...
export const Camera = forwardRef<CameraRef, CameraProps>(
  (
    ...
    const tr = (fr: string, ar: string, en: string) => en; // Default to English for UI kit
    ...
    if (!permission.granted || (enableVideo && micPermission && !micPermission.granted)) {
      return (
        <View
          style={[styles.permissionContainer, { backgroundColor: cardColor, padding: 24, justifyContent: 'center', alignItems: 'center' }]}
        >
          <CameraIcon
            size={48}
            color={primaryColor}
          ...
          />
          <Text variant='title' style={{ textAlign: 'center', fontSize: 22, fontWeight: '700' }}>
            {tr('Autorisation requise', 'الموافقة مطلوبة', 'Permission Required')}
          </Text>
          <Text variant='body' style={{ textAlign: 'center', marginTop: 12, marginBottom: 32, opacity: 0.7 }}>
            {tr(
              "Nous avons besoin de votre caméra et de votre micro pour capturer des moments.",
              "نحتاج إلى الكاميرا والميكروفون لالتقاط اللحظات.",
              "We need access to your camera and microphone to capture moments."
            )}
          </Text>
          <Button
            onPress={async () => {
              await requestPermission();
              if (enableVideo) await requestMicPermission();
            }}
            style={{ width: '100%', borderRadius: 16, height: 54 }}
          >
            {tr('Autoriser', 'السماح', 'Grant Permission')}
          </Button>
          <Button
            variant="ghost"
            onPress={onClose}
