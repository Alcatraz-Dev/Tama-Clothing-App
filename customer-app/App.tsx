...
export default function App() {
  ...
  const handleScan = async (data: string) => {
    ...
    } else if (data.startsWith('BEY3A-') || (activeShipment && data === activeShipment.trackingId)) {
  ...
  const renderMainContent = () => {
    ...
    switch (activeTab) {
      ...
      case 'Orders': return <CommandScreen onBack={() => setActiveTab('Profile')} onTrack={(tid: string) => { setActiveTrackingId(tid); setActiveTab('ShipmentTracking'); }} t={t} language={language} />;
      ...
      case 'AdminDeliveryCompanies': return <AdminDeliveryCompaniesScreen onBack={() => setActiveTab('AdminMenu')} t={t} />;
      case 'AdminFinanceDashboard': return <AdminFinanceDashboard onBack={() => setActiveTab('AdminMenu')} t={t} />;
      case 'BrandRevenue': return <BrandRevenueScreen onBack={() => setActiveTab('AdminMenu')} profileData={profileData} t={t} />;
      ...
      case 'ShipmentTracking': return <ShipmentTrackingScreen
        ...
        t={t}
      />;
      case 'MyShipments': return <MyShipmentsScreen
        onBack={() => setActiveTab('Profile')}
        onTrackShipment={(trackingId: string) => { setActiveTrackingId(trackingId); setActiveTab('ShipmentTracking'); }}
        t={t}
        user={user}
        profileData={profileData}
      />;
      ...
      case 'DynamicIslandTest': return <DynamicIslandTestScreen
        onBack={() => setActiveTab('Profile')}
      />;
          ...
          {isBrandOwner && (
            <View style={{ position: 'absolute', top: '30%', left: 0, right: 0, alignItems: 'center', transform: [{ translateY: -15 }] }}>
              ...
              <View style={{
                ...
                <View>
                  <Text style={{ color: theme === 'dark' ? '#FFF' : '#000', fontSize: 14, fontWeight: '900', letterSpacing: 0.5 }}>{getName(brandInfo?.name, 'BEY3A BRAND').toUpperCase()}</Text>
                ...
                {(profileData?.role === 'brand_owner' || profileData?.role === 'admin') && (
                  <TouchableOpacity style={[styles.menuRow, { paddingVertical: 18, borderBottomColor: colors.border }]} onPress={() => setActiveTab('ShipmentCreation')}>
                    <View style={styles.menuRowLeft}>
                      ...
                      <Text style={[styles.menuRowText, { color: colors.foreground }]}>{t('createNewShipment')}</Text>
                    </View>
                    <ChevronRight size={18} color={colors.textMuted} />
                  </TouchableOpacity>
                )}

                <TouchableOpacity style={[styles.menuRow, { paddingVertical: 18, borderBottomColor: colors.border }]} onPress={() => setActiveTab('MyShipments')}>
                  <View style={styles.menuRowLeft}>
                    <View style={[styles.iconCircle, { backgroundColor: theme === 'dark' ? '#17171F' : '#F9F9FB' }]}>
                      <Truck size={20} color={colors.accent} />
                    </View>
                    <Text style={[styles.menuRowText, { color: colors.foreground }]}>{t('myShipments')}</Text>
                  </View>
                  <ChevronRight size={18} color={colors.textMuted} />
                </TouchableOpacity>

                {profileData?.role === 'admin' && (
                  <TouchableOpacity style={[styles.menuRow, { paddingVertical: 18, borderBottomColor: colors.border }]} onPress={() => setActiveTab('DynamicIslandTest')}>
                    <View style={styles.menuRowLeft}>
                      <View style={[styles.iconCircle, { backgroundColor: theme === 'dark' ? '#17171F' : '#F9F9FB' }]}>
                        <LayoutDashboard size={20} color="#FF2D55" />
                      </View>
                      <Text style={[styles.menuRowText, { color: colors.foreground }]}>TEST DYNAMIC ISLAND</Text>
                    </View>
                    <ChevronRight size={18} color={colors.textMuted} />
                  </TouchableOpacity>
                )}
...
function CartScreen({ cart, onRemove, onUpdateQuantity, onComplete, profileData, updateProfile, onBack, t }: any) {
  ...
  const placeOrder = async () => {
    ...
    try {
      ...
      try {
        const { createShipmentFromOrder } = await import('./src/utils/shipping');
